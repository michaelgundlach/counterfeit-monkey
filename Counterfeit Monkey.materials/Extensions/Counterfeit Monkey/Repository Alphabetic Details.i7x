Repository Alphabetic Details by Counterfeit Monkey begins here.
...
The Shadow Chamber is a room. It is indoors, checkpoint and forbidden. The description is "Dim lights in the floor make it possible to navigate in here, though it's still fairly dark. The place is small and shabby, the air stale.[assign-amanda]".
...
Understand "attach [nib] to [pen]" as inserting it into.
...
After reading a command when the player carries the pill or the player carries the pills (this is the replace take pill with eat pill rule):
	if the player's command matches "take pill":
		if the player carries the pill:
			replace the player's command with "eat pill";
	if the player's command matches "take pills":
...
Roc-flight is a scene. Roc-flight begins when the roc is in location. Roc-flight ends when the player is held by the roc.
...
Sanity-check waiting during roc-soaring:
	say "That is about the only thing [we] can do." instead.
...
Instead of jumping when the player is on the trampet:
	if there is a thing which is not the player on the trampet and a random chance of 1 in 3 succeeds:
		...
		say "[The random-thing] [bounce] off the trampet.[paragraph break]";
...
Instead of inserting something into the trap:
	try springing the trap with the noun.
